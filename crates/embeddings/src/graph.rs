...
impl LinkGraph {
    ...
    pub fn add_link(&mut self, path_a: &Path, path_b: &Path) {
        ...
        self.adj
            .entry(path_a.to_path_buf())
            .or_default()
            .insert(path_b.to_path_buf());
        self.adj
            .entry(path_b.to_path_buf())
            .or_default()
    ...
    pub fn all_linked_pairs(&self) -> HashSet<PathPair> {
        ...
        for (path_a, neighbors) in &self.adj {
            for path_b in neighbors {
                if path_a < path_b {
                    // each link appears twice, so only add one direction
    ...
    pub fn from_notes(notes: &[Note], exclude_prefixes: &[PathBuf]) -> Self {
        ...
        let all_paths: HashSet<PathBuf> = notes
            .iter()
            .filter(|n| !is_excluded(&n.path, exclude_prefixes))
            .map(|n| n.path.clone())
            .collect();

        let title_to_path: HashMap<String, PathBuf> = notes
            .iter()
            .filter(|n| !is_excluded(&n.path, exclude_prefixes))
...
mod tests {
    ...
    fn simple_wikilink() {
        assert_eq!(
            parse_wikilinks("See [[Meeting Notes]] for details"),
            vec!["Meeting Notes"]
        );
    }
    ...
    fn wikilink_with_alias() {
        assert_eq!(
            parse_wikilinks("Check [[vault/projects/this doc.md|Project Alpha]]"),
            vec!["vault/projects/this doc.md"]
        );
    }
    ...
    fn all_linked_pairs_returns_hashset() {
        ...
        assert!(pairs.contains(&PathPair::new(
            &PathBuf::from("a.md"),
            &PathBuf::from("b.md")
        )));
        assert!(pairs.contains(&PathPair::new(
    ...
    fn resolve_link_not_found() {
        let all_paths: HashSet<PathBuf> =
            vec![PathBuf::from("Projects/App.md")].into_iter().collect();
    ...
    fn resolve_link_path_before_title() {
        let all_paths = vec![PathBuf::from("Work/Meeting Notes.md")]
            .into_iter()
            .collect();

        let mut title_to_path = HashMap::new();
        title_to_path.insert(
            "Meeting Notes".to_string(),
            PathBuf::from("Personal/Meeting Notes.md"),
        );
